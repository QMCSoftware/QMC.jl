...
Download the matching source distribution and wheel from PyPI or the qmctoolscl
GitHub release page:
...
This drops both the `.tar.gz` source distribution and the platform wheels into
`/tmp/qmc_dl`.
...
Only the C sources under `qmctoolscl/c_funcs/` are used at build time;
the rest of the Python package is ignored.
...
Verify the file does **not** import `python312.dll` (it must load as a plain
shared library without a Python runtime):
...
Compare the C function list in `upstream/qmctoolscl-X.Y.Z/qmctoolscl/c_funcs/`
against the previous version. If new functions were added that QuasiMC should
use, wire them up in `src/discrete_distribution/qmctoolscl_c.jl`. If new fused
or vectorised variants appeared, check whether the `_HAS_DNB2_FUSED` probe in
that file still applies or needs a matching update.
...
Trigger the `install-test.yml` workflow (or run it locally via `make workflow-smoke`)
on Linux, macOS, and Windows to confirm a clean install with no Python dependency
