...
publish_once() {
  ...
  (
    ...
    git commit -m "Update coverage badge ${scope_slug} for ${GITHUB_SHA}"
    git push origin HEAD:"$BADGE_BRANCH"
  )
}

for attempt in 1 2 3 4 5; do
  if publish_once "$tmpdir/attempt-$attempt"; then
    exit 0
  fi
  echo "Retrying coverage badge publish after attempt ${attempt} failed" >&2
  sleep "$attempt"
done
