...
To keep the benchmark coverage badge above the repository target without distorting performance badges, the `bench-*-coverage` targets also enable a `BENCH_COVERAGE=1` probe pass from `devtools/coverage_probes.jl`. Those probes exercise low-coverage `src/` branches such as control variates, additional `FinancialOption` variants, `DigitalNetB2` validation/loading paths, and lattice resume/diagnostics flows. They are coverage-only checks, not timing rows, and they are not used by the plain `make bench-all` badge pipeline.
