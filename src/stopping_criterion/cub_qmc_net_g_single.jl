...
f = Keister(tm)
sc = CubQMCNetGSingle(f; abs_tol=1e-3)
result = integrate(sc)
```
"""
...
function CubQMCNetGSingle(
    ...
    dd.replications === nothing || error(
        "CubQMCNetGSingle requires a non-replicated net (construct DigitalNetB2 with replications=nothing).",
    )
    !dd.graycode || error(
        "CubQMCNetGSingle requires natural (radical-inverse) order: construct DigitalNetB2 with graycode=false.",
    )
    ispow2(n_init) || error("CubQMCNetGSingle: n_init must be a power of two.")
    n_floor = 2^max(r_lag + 1, 8)
    if n_init < n_floor
        @warn "CubQMCNetGSingle: n_init bumped up to $n_floor (the guaranteed bound needs n_init ≥ 2^max(r_lag+1, 8))."
        n_init = n_floor
    end
    return CubQMCNetGSingle(integrand, abs_tol, rel_tol, n_init, n_max, r_lag, trace_iterations)
end
...
function integrate(sc::CubQMCNetGSingle; resume::Union{Nothing, Dict{Symbol, Any}}=nothing)
    ...
    while n <= sc.n_max
        ...
        if ndims(x_unit) == 3
            size(x_unit, 1) == 1 || error("CubQMCNetGSingle requires a non-replicated net.")
